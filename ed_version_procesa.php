<?php 
/**
* ed_version_procesa.php
*
* aplicación para procesar una versión candidata generando un nuevo cuerpo de informaicón en la bvase de datos espacial
 * 
 *  
...
if($Log['data']['version']['id']!=$_POST['id']){
	$Log2['tx'][]='error al validar el id de version como ultima versión no publicada para este usuario y esta version';
	$Log2['res']='err';
	terminar2($Log2);	
}
...
while($carga<1000000){	
	...
	*/
	...
	foreach($Log['data']['columnas'] as $tnom => $ttipo){
		...
		$valores.="'".utf8_encode($reg['dbf'][$nrefdbf])."', ";
	...
	if(pg_errormessage($ConecSIG)!=''){
		$Log2['res']='error';
		$Log2['tx'][]='error al borrar base de datos';
